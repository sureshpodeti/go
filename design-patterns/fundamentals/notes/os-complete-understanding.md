...
The CPU never talks to hardware directly in a raw, unmediated way. Every communication follows a specific path — through buses, controllers, and the OS kernel. Understanding these paths is what separates engineers who guess at performance from those who reason about it.

---

#### CPU ↔ RAM

**The problem the CPU faces**

Your Go code says `x := mySlice[0]`. The CPU needs the value at some memory location. It knows the *virtual address* — say `0x7fff1234`. But that address is fiction. It doesn't correspond to actual wires on the RAM chip. The CPU needs the *physical address* — the real location on the hardware.

**Step 1 — The MMU translates the address (inside the CPU)**

The MMU (Memory Management Unit) is a hardware circuit *inside* the CPU die — not a separate chip. Every memory access automatically passes through it. The MMU holds a reference to the current process's page table via the CR3 register. It splits the virtual address into a page number and an offset, looks up the page number in the page table, finds the physical frame, and combines it with the offset to produce the physical address.

But walking the full page table takes 4 RAM accesses on x86-64 (it's a 4-level structure). That would be catastrophic — every memory access requiring 4 more memory accesses just to translate. So the MMU has a small hardware cache called the **TLB** (Translation Lookaside Buffer) — typically 64–1024 entries — that caches recent translations.

```
CPU generates virtual address 0x7fff1234
         │
         ▼
    Check TLB (hardware cache of recent translations)
         │
    ┌────┴────────────────────────────────────────┐
    │ TLB HIT                    TLB MISS         │
    │ (~1ns extra)               (~400ns extra)   │
    │ physical addr              walk 4-level      │
    │ found directly             page table in RAM │
    └────┬────────────────────────────────────────┘
         │
         ▼
    Physical address (e.g. 0x2A001234)
```

This is also why context switches are expensive — switching processes flushes the TLB. The new process starts cold and pays the page table walk cost on its first accesses.

**Step 2 — The memory controller talks to the RAM chips**

Once the MMU has the physical address, it goes to the **memory controller** — on modern CPUs this is integrated directly into the CPU die (it used to be a separate chip called the northbridge). The memory controller is the hardware that knows how to speak DDR protocol to the actual DRAM chips. It puts the physical address on the memory bus and issues a read command.

**Step 3 — Why 64 bytes, not just the 8 bytes you asked for**

You asked for one `int64` — 8 bytes. But the CPU fetches **64 bytes** (a full cache line). This is because of *spatial locality* — if you access address X, you will almost certainly access X+8, X+16, X+24 soon after (iterating a slice, reading struct fields). By fetching 64 bytes at once, the CPU pre-loads neighboring data into cache. The next 7 accesses after the first are cache hits — essentially free.

The 64-byte cache line is the atomic unit of transfer between RAM and cache. You cannot fetch less than 64 bytes. The memory controller always transfers a full cache line.

**Step 4 — The data bus width vs cache line size**

The data bus between CPU and RAM is 64 bits wide — 8 bytes per transfer cycle. A cache line is 64 bytes. So fetching one cache line requires 8 consecutive bus cycles. Modern DDR memory handles this as a *burst transfer* — you specify the starting address and it automatically streams the next 8 chunks without needing separate commands for each.

**Where the ~100ns goes**

```
Memory controller receives request, issues DRAM command    ~10ns
DRAM row activation (capacitors charge up the right row)   ~30-40ns
Column access (select right columns within the row)        ~10ns
Data transfer back over memory bus (8 burst cycles)        ~15-20ns
Load into L3 → L2 → L1 cache                              ~10ns
─────────────────────────────────────────────────────────────────
Total                                                      ~100ns
```

At 3GHz, one CPU cycle is 0.33ns. So 100ns = **~300 CPU cycles** where the CPU is stalled waiting. Modern CPUs use out-of-order execution to try to do other independent work during this wait, but if the next instruction depends on the result of this load, the CPU stalls completely.
...
CPU core
  ...
  ▼
MMU (inside CPU)
  │  TLB hit → physical address in ~1ns
  │  TLB miss → page table walk → ~400ns
  ▼
Memory Controller (inside CPU die)
  │  DDR command on memory bus
  ▼
RAM chips (DIMM slots on motherboard)
  │  burst transfer: 64 bytes in 8 cycles
  ▼
L3 cache → L2 cache → L1 cache → CPU register
  total round trip: ~100ns on cache miss
```
...
**The CPU never talks to disk directly**

This is a critical mental model. There is no direct wire from the CPU to the disk. The path always goes through the OS kernel and a device driver. Your code cannot issue a disk read — it must ask the kernel to do it via a system call.

**Why this indirection exists**

If user code could talk to disk directly, any program could read any file on the disk regardless of permissions. The kernel sits in the middle to enforce access control, manage concurrent access from multiple processes, and provide a uniform interface (the file abstraction) over wildly different hardware (NVMe, SATA SSD, HDD, network filesystem).

**The full path when you call `os.ReadFile("data.txt")`**

```
Your Go code: os.ReadFile("data.txt")
    │
    │  Go runtime calls read() system call
    │  CPU traps to kernel (Ring 3 → Ring 0)
    ▼
Kernel — VFS (Virtual File System) layer
    │  VFS is an abstraction: same API for ext4, APFS, NFS, etc.
    │  Checks file permissions (does this process own this file?)
    │  Checks page cache: is this file's data already in RAM?
    │
    ├── PAGE CACHE HIT: data already in RAM from a previous read
    │     Copy from kernel buffer to user buffer (copy_to_user)
    │     Return to user code — no disk involved at all
    │
    └── PAGE CACHE MISS: data not in RAM, must read from disk
          │
          ▼
        Block Device Driver (e.g. NVMe driver, AHCI driver)
          │  Translates file offset to physical block address
          │  Submits I/O request to disk controller
          ▼
        Disk Controller (hardware on PCIe bus)
          │  Issues read command to the physical disk
          ▼
        Physical Disk (SSD/HDD)
          │  SSD: reads NAND flash cells (~100µs)
          │  HDD: moves arm, waits for rotation (~10ms)
          ▼
        DMA Transfer — disk controller writes data DIRECTLY to RAM
          │  CPU is NOT involved in moving the bytes
          │  CPU is free to run other goroutines during this time
          ▼
        Disk controller sends hardware interrupt to CPU
          │  "I'm done, data is in RAM at address X"
          ▼
        Kernel wakes up the blocked goroutine/process
          │  copy_to_user: copies from kernel buffer to user buffer
          ▼
        Your Go code receives the data and continues
```

**DMA is the key insight**

Without DMA, the CPU would have to execute a loop: read one byte from the disk controller register, write it to RAM, repeat for every byte. For a 4KB read that's 4096 iterations — the CPU is fully occupied just moving bytes. With DMA, the disk controller has its own bus master capability and writes directly into RAM. The CPU sets up the transfer (source, destination, size) and then goes off to do other work. This is why disk I/O can be non-blocking — the CPU is genuinely free while the transfer happens.
...
When your Go code calls `conn.Write(data)`, the kernel's TCP/IP stack builds a packet and needs to hand it to the NIC. It does this through a **TX ring buffer** — a circular queue of packet descriptors in RAM that both the CPU and NIC can see.

```
Your Go code: conn.Write(data)
    │
    ▼
Kernel TCP/IP stack
    │  Wraps data in TCP segment → IP packet → Ethernet frame
    │  Writes packet descriptor into TX ring buffer (in RAM)
    │  Writes to NIC's doorbell register: "new packet ready"
    ▼
NIC hardware
    │  Reads packet descriptor from TX ring buffer via DMA
    │  Reads actual packet data from RAM via DMA
    │  Serializes bits onto the wire (copper/fiber)
    ▼
Network
```

The CPU's job is just to write a small descriptor into the ring buffer and ring the doorbell. The NIC does the rest via DMA. The CPU is free immediately.

**Receiving a packet**

```
Network → NIC hardware
    │  NIC receives bits from wire
    │  Assembles into Ethernet frame
    │  Writes frame into RX ring buffer in RAM via DMA
    │  Sends hardware interrupt to CPU: "packet arrived"
    ▼
CPU interrupt handler (in kernel)
    │  Reads packet from RX ring buffer
    │  Passes up through network stack: Ethernet → IP → TCP
    │  Copies data to the socket's receive buffer
    │  Wakes up any goroutine blocked on conn.Read()
    ▼
Your Go code: conn.Read(buf) returns with data
```

**The interrupt problem at high load**

At 10 Gbps, a NIC can receive millions of small packets per second. If it sends one interrupt per packet, the CPU spends all its time handling interrupts and has no time to actually process the data. Modern NICs use **NAPI** (New API in Linux) — after the first interrupt, the kernel switches to *polling* mode: it keeps checking the RX ring buffer in a tight loop until it's empty, then goes back to interrupt mode. This batches the interrupt overhead across many packets.

```
Low traffic:   interrupt per packet (low latency, low CPU)
High traffic:  one interrupt → poll until ring empty (high throughput)
```

This is also why Go's `net` package is efficient — the Go runtime uses `epoll` (Linux) / `kqueue` (macOS) to monitor many connections with a single OS thread, rather than blocking one thread per connection.

---

#### CPU ↔ Keyboard

**The keyboard is interrupt-driven — and that's the right mental model for all I/O**

The CPU does not sit in a loop asking "was a key pressed? was a key pressed? was a key pressed?" — that would waste 100% of a CPU core just watching the keyboard. Instead, the keyboard controller interrupts the CPU only when something actually happens.

```
Nothing happening: CPU runs your program at full speed, keyboard ignored

User presses 'A':
    │
    ▼
Keyboard controller (microcontroller inside the keyboard)
    │  Detects key press, generates scan code (e.g. 0x1E for 'A')
    │  Asserts IRQ1 line (Interrupt Request line 1)
    ▼
CPU's interrupt controller (APIC — Advanced Programmable Interrupt Controller)
    │  Receives IRQ1
    │  Signals CPU: "interrupt pending"
    ▼
CPU finishes its current instruction (cannot interrupt mid-instruction)
    │  Saves current state: pushes PC, flags, registers onto kernel stack
    │  Switches to Ring 0 (kernel mode)
    │  Looks up IRQ1 in the Interrupt Descriptor Table (IDT)
    │  Jumps to the keyboard interrupt handler
    ▼
Kernel keyboard interrupt handler
    │  Reads scan code from I/O port 0x60 (one byte)
    │  Translates scan code → key code → ASCII/Unicode character
    │  Puts character into the terminal's input buffer
    │  Wakes up any process blocked reading from the terminal
    ▼
CPU executes IRET instruction
    │  Restores saved state (PC, flags, registers)
    │  Switches back to Ring 3 (user mode)
    ▼
Your program resumes exactly where it was interrupted
    (it never knew the interrupt happened)
```

**Why this matters architecturally**

The interrupt model is the foundation of all efficient I/O. The CPU is never polling. It is always doing useful work. Hardware signals the CPU only when something needs attention. This same pattern — interrupt → handler → wake up waiting process — applies to disk completion, network packet arrival, timer expiry, and every other hardware event. The OS is fundamentally an interrupt-driven system.
